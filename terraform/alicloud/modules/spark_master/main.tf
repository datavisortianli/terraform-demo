...
resource "alicloud_instance" "spark_master" {
...
resource "alicloud_disk" "spark_master_xvdb" {
  count = "${var.instance_count}"
  availability_zone = "${element(split(",", var.availability_zones), count.index)}"
...
resource "alicloud_disk_attachment" "spark_master_xvdb_attach" {
  count = "${var.instance_count}"
  disk_id = "${element(alicloud_disk.spark_master_xvdb.*.id, count.index)}"
  instance_id = "${element(alicloud_instance.spark_master.*.id, count.index)}"
